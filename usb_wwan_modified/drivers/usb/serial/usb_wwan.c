				      ...
				      void (*callback) (struct urb *))
{
	...
	printk(KERN_INFO "usb_wwan.c: buf = %s\n",buf);
...
int usb_wwan_resume(struct usb_serial *serial)
{
	...
	for (i = 0; i < serial->num_ports; i++) {
		...
		portdata = usb_get_serial_port_data(port);
		printk(KERN_INFO "usb_wwan.c: usb_wwan_resume(): portdata->in_buffer = %s, portdata->out_buffer = %s\n", portdata->in_buffer, portdata->out_buffer);
