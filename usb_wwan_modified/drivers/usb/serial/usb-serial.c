...
{
	int i;
	printk(KERN_INFO, "usb-serial.c: usb_serial_port_poison_urbs\n");
...
static void usb_serial_port_unpoison_urbs(struct usb_serial_port *port)
{
	...
	printk(KERN_INFO, "usb-serial.c: usb_serial_port_unpoison_urbs\n");
	for (i = 0; i < ARRAY_SIZE(port->read_urbs); ++i)
			       ...
			       const struct usb_device_id *id)
{
	...
	printk(KERN_INFO "usb-serial.c: setting up %d port structure(s)\n", max_endpoints);
	...
	for (i = 0; i < num_bulk_in; ++i) {
		...
		for (j = 0; j < ARRAY_SIZE(port->read_urbs); ++j) {
			...
			usb_fill_bulk_urb(port->read_urbs[j], dev,
					...
					port);
			printk(KERN_INFO "usb-serial.c: port->bulk_in_buffers[%d]=%s\n", port->bulk_in_buffers[j]);
		}
	...
	for (i = 0; i < num_bulk_out; ++i) {
		...
		for (j = 0; j < ARRAY_SIZE(port->write_urbs); ++j) {
			...
			usb_fill_bulk_urb(port->write_urbs[j], dev,
					...
					port);
			printk(KERN_INFO "usb-serial.c: port->bulk_out_buffers[%d]=%s\n", port->bulk_out_buffers[j]);
		}
